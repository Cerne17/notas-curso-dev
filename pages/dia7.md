...
```mermaid
graph BT
    %% Definição dos Nós e Fluxo (de baixo para cima)
    c["Alterações Locais"] --> b["Alterações no Repositório Remoto"]
    b --> a["'Robozinho'"]

    %% Estilização para ficar menos "bruto"
    style a fill:#cc8ce6,stroke:#333,stroke-width:2px,color:#000
